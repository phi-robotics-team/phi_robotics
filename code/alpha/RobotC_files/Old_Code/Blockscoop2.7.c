...
#pragma config(Motor,  mtr_S1_C1_1,     F_L,           tmotorTetrix, PIDControl, encoder)
...
task Score_task()
{
 while(true)
  ...
  if (joy1Btn(BTN_LB))
	...
	if (joy1Btn(BTN_LT))
	{
		while(joy1Btn(BTN_LT))
		{
			motor[Block_Sweep] = -100;
		}
		motor[Block_Sweep] = 0;
// Bucket Functions
  if (joystick.joy1_TopHat == HAT_UP)
  {
  	while (joystick.joy1_TopHat == HAT_UP)
  	{
  		motor[Jacklift1] = 100;
  		motor[Jacklift2] = 100;
  	}
	...
	if (joystick.joy1_TopHat == HAT_DOWN)
	{
		while (joystick.joy1_TopHat == HAT_DOWN)
		{
			motor[Jacklift1] = -100;
  	  motor[Jacklift2] = -100;
  	}
