...
task drive()
{
	...
	while (true)
	{
		...
		getJoystickSettings(joystick); //Gets value from the joystick
		 ...
		 oldDirection = SensorValue[Compass];
		 ...
		 while (SensorValue[Compass] != newDirection +- deadband/4)
		 {
		   motor[F_L] = sgn(MoveX) * Magnatude;
		   motor[B_L] = sgn(MoveX) * Magnatude;
		   motor[F_R] = sgn(MoveX) * -Magnatude;
		   motor[B_R] = sgn(MoveX) * -Magnatude;
	   }
