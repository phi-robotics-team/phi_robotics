...
void initializeRobot()
{
	...
	nMotorPIDSpeedCtrl[Arm]= mtrSpeedReg/4;
	nMaxRegulatedSpeed12V = 1000;
	servo[Bucket_Release] = 0;
	servo[Spring_Release] = 240;
...
task Arm_task()
{
  ...
  while (true)
	{
		getJoystickSettings(joystick);
		if (abs(joystick.joy1_y2) > deadband1)
		{
			motor[Arm] = -Gain1* (joystick.joy1_y2 - deadband) * abs(joystick.joy1_y2 - deadband);
		}
		else
		{
			motor[Arm] = 0;
...
//********************************
// Flag Raising Mechanisms       *
//********************************
		if (joy1Btn(BTN_A))
		{
			if(servo[Spring_Release] != 0)
			{
...
//********************************
// Bucket Releasing Mechanism   *
//********************************
		if (joy1Btn(BTN_RT))
		{
			if (servo[Bucket_Release] == 0)
			{
				servo[Bucket_Release] = 255 +- 20;
				wait10Msec(20);
			}
			else if (servo[Bucket_Release] == 255 +- 20)
			{
				servo[Bucket_Release] = 0;
				wait10Msec(20);
			}
