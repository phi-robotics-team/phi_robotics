...
void Move( float distance, float max_speed )
{
  ...
  while (abs(nMotorEncoder[F_R]) < countDistance && abs(nMotorEncoder[F_L])  <  countDistance && abs(nMotorEncoder[B_L])<  countDistance && abs(nMotorEncoder[B_R])<  countDistance) //while the encoder wheel turns one revolution
  {
    ...
    if (Sum_left > Sum_right)
    {
		...
		if (abs(nMotorEncoder[F_R]) >= countDistance - Prencodercount && abs(nMotorEncoder[F_L])  >=  countDistance - Prencodercount && abs(nMotorEncoder[B_L]) >=  countDistance - Prencodercount && abs(nMotorEncoder[B_R]) >=  countDistance - Prencodercount)
		{
			  motor[B_L] = MAX_ENCODER_SPEED/2;
  	  	motor[B_R] = MAX_ENCODER_SPEED/2;
  		  motor[F_L] = MAX_ENCODER_SPEED/2;
        motor[F_R] = MAX_ENCODER_SPEED/2;
      }
