...
task main()
{
	...
	while(true)
	{
		...
		else//if the Message change you are connected
		{
		     ...
		     nxtDisplayTextLine(1, "CONNECTED");	//displays CONNECTED to tell you if everything is working
...
task drive()
{
	...
	while (true)
	{
     //nxtDisplayTextLine(2, "Drive CONNECTED");
		...
		Joy1Ymve = (Joy1Ymve * Gain1) + (Joy1Ymve * abs(Joy1Ymve) * Gain2);
		Joy1Xmve = (Joy1Xmve * Gain1) + (Joy1Xmve * abs(Joy1Xmve) * Gain2);
...
task Arm_task()
{
  ...
  while (true)
	{
		getJoystickSettings(joystick);
       //  nxtDisplayTextLine(3, "arm CONNECTED");
		...
		if (abs(armPower_raw) > deadband)
		{
			...
			armPower = (armPower_raw/cut)*abs(armPower_raw/cut);
			if(armPower > limit)
			{
				armPower = limit;
			}
		...
		targetHeight = nMotorEncoder[Arm];
		armPower = 0;
		}
		...
		motor[Arm] = 0;
		/*	error = targetHeight - nMotorEncoder[Arm];
		armPower += (int)(Kp*error);
		if(abs(error) < 50)
		{
		armPower = 0;
		}
		StringFormat(s, "error = %d", error);
