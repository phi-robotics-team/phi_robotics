...
task drive()
{
	...
	while (true)
	{
		...
		Joy1Ymve = (Joy1Ymve * Gain1) + ((Joy1Ymve * Gain2) * (abs(Joy1Ymve) * Gain3));
		Joy1Xmve = (Joy1Xmve * Gain1) + ((Joy1Xmve * Gain2) * (abs(Joy1Xmve) * Gain3));
  ...
  while (true)
	{
		...
		if (abs(armPower_raw) > deadband)
		{
			...
			armPower = (armPower * Gain1) + ((armPower * Gain2) * (abs(armPower) * Gain3));
