...
void Turn( float turnDegrees )
{
  ...
  motorSpeed = 5;
  motor[leftDrive] = -rotDirection * motorSpeed;
  motor[rightDrive] = rotDirection * motorSpeed;
  wait1Msec(10);
  ...
  while( ( abs(turnDegrees) - abs(Angle) ) > 3 )
  {
    ...
    motor[leftDrive] = rotDirection * motorSpeed ; // looking from the top, left side moves back, right
    motor[rightDrive] = -rotDirection * motorSpeed ; // side moves forward for (+) CCW rotation
