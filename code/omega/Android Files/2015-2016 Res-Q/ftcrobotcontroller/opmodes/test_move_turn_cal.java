...
public class test_move_turn_cal extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            switch (state) {
              /** case 0:
                    //trade tolerance for when compensate is active, tdly. compvar, gain for move and compensate functions,
                    // Might improve precision with backlash compensation and ramping down power as you get close to targeted distance
                    dist = 45.0;
                    power = 0.5;
                    ...
                    break;*/
               case 0:
                    // test the while loop tolerance on when to stop turning
                    reqTurnDegrees = -45;
                    Turn(reqTurnDegrees);
                    telemetry.addData("state", state);
                    state = state + 1;
...
*/
    ...
    void Turn(double turnDegrees) throws InterruptedException {
        ...
        double MIN_GYRO_SPEED = .20; //minimum motor speed allowed in turns
       ...
       /* double motorSpeed = .05;
        ...
        while (((rightFrontCurrentEncoder + leftFrontCurrentEncoder) / 2 < Math.abs(cnt_target)) && ((rightRearCurrentEncoder + leftRearCurrentEncoder) / 2 < Math.abs(cnt_target))) {
           ...
           /* telemetry.addData("leftFront Power", leftFront.getPower());
            ...
            telemetry.addData("Average_Front_Encoder",(rightFrontCurrentEncoder + leftFrontCurrentEncoder) / 2);
            telemetry.addData("Average_Rear_Encoder",(rightRearCurrentEncoder + leftRearCurrentEncoder) / 2);
            telemetry.addData("Count_Target",cnt_target);
            telemetry.addData("compensate",compensate);
            DbgLog.msg(String.format("Data3-Average_Front_Encoder %.03f", (float) (rightFrontCurrentEncoder + leftFrontCurrentEncoder) / 2));
            DbgLog.msg(String.format("Data3-Average_Rear_Encoder  %.03f", (float) (rightRearCurrentEncoder + leftRearCurrentEncoder) / 2));
            DbgLog.msg(String.format("Data3-Count_Target %.03f", (float) cnt_target));
            DbgLog.msg(String.format("Data3-compensate %.03f", (float) compensate));
