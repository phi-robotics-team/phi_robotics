...
task main()
{
  ...
  while (true)
  {
  	...
  	if (nxtButton == 1 && servoVal < 255)
  		servoVal += 3;
  	else if (nxtButton == 2 && servoVal > 0)
  		servoVal -= 3;
  	servo[irTurret] = servoVal;

  	eraseDisplay();
  	nxtDisplayCenteredBigTextLine(1, "S: %d", servoVal);
  	nxtDisplayCenteredBigTextLine(4, "IR: %d", SensorValue[ir]);
