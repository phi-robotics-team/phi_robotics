...
task main()
{
   ...
   while ( time1[T1] < 3000)
   {
     if (nNxtButtonPressed == 1 && waitTime < 9)
   	{
  	   waitTime++;
  	   ClearTimer(T1);
  	 }
  	 else if (nNxtButtonPressed == 2 && waitTime > 0)
   	{
  	   waitTime--;
  	   ClearTimer(T1);
  	 }
  	 nxtDisplayBigTextLine(1, "%d, %s", waitTime, "sec");
  	 wait10Msec(30);
  }
 ...
 wait10Msec(waitTime*100);

   if(IRvalue==3)// near side of board, or column A
	{
  ...
  else if(IRvalue==5) // far side, or column C
	{
   ...
   Move(-33.0, 70.0);    // drive backward 33 inches
   wait10Msec(10);

   Turn_gyro(45.0);     // turn counterclockwise 45 degrees
   wait10Msec(10);

   Move(-21.0, 70.0);    // drive backward 21 inches
   wait10Msec(10);
