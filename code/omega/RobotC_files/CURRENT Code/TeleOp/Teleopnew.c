...
#pragma config(Servo,  srvo_S1_C4_3,    aim,                  tServoStandard)
...
task changeArm()
{
	while (true)
	{
		if(joy1Btn( BTN_SELECT ))
		{
			...
			if (mode == 2) //If in quick control mode
			{
				mode = 1; //Select button switches controls to slow controls
			}
			else
			{
...
task Arm1()
{
	...
	while (true)
	{
		...
		if(joy1Btn(BTN_RT))
		{
			if (ServoValue[rake] <= 80)
			{
				servo[rake] = 255;
			}
			else if (ServoValue[rake] == 255)
			{
				servo[rake] = 80;
			}
		}

		while (joystick.joy1_TopHat == HAT_LEFT)
		{
			servo[aim] = ServoValue[aim] + 5;
		}

		while (joystick.joy1_TopHat ==HAT_RIGHT)
		{
			servo[aim] = ServoValue[aim] - 5;
		}
