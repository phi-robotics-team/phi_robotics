...
#pragma config(Motor,  mtr_S1_C1_1,     L_Rear,        tmotorTetrix, openLoop)
...
#pragma config(Motor,  mtr_S1_C3_1,     R_Front,       tmotorTetrix, openLoop, reversed, encoder)
#pragma config(Motor,  mtr_S1_C3_2,     L_Front,       tmotorTetrix, openLoop, encoder)
...
//	bool clawOpen = true;
		...
		while(abs(joystick.joy1_y2) > deadband)
    {
    	...
    	armSpeed = ((float)joystick.joy1_y2 / 127.0) * 10;
...
/*
    ...
    if( (nMotorEncoder[Arm_motor] - armPosition) < -5)
    {
      armSpeed = 20;
      motor(Arm_motor) = armSpeed;
    }

   	else if( (nMotorEncoder[Arm_motor] - armPosition) > 5)
    {
      armSpeed = -20;
      motor(Arm_motor) = armSpeed;
    }
...
*/
		...
		/*if(joy1Btn( 1 ))			// If button 1 (X) is pressed, go to End Game
		{
			servo[Ramp] = 255;
			wait10Msec(100);
			servo[Ramp] = 0;
		...
		if(joy1Btn( 3 ))			// If button 3 (B)  is pressed, return to Drive
 	{
		StartTask( drive );
	}*/
  ...
  if (joy1Btn(4))
  {
  	if (nMotorEncoder[Arm_motor] > armEncoder)
  	{
  		motor[Arm_motor] = -50;
  	}
  	else if (nMotorEncoder[Arm_motor] < armEncoder)
  	{
  		motor[Arm_motor] = 50;
  	}
  	while (abs(nMotorEncoder[Arm_motor]-armEncoder) >= 100 && !joy1Btn(2))
  	{
  	}
...
task drive()
{
	...
	while (true)
	{
		...
		if(joy1Btn( 8 ))			// If button 8 (bottom right trigger) is pressed, turn wrist one way
 	{
			...
			{
				servo[L_Wrist] = ServoValue[L_Wrist]-8;
				servo[R_Wrist] = ServoValue[R_Wrist]+8;
			}
	}
