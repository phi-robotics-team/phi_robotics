...
#pragma config(Motor,  mtr_S1_C1_2,     fish,          tmotorTetrix, openLoop, reversed)
...
{
	//servoChangeRate[R_Wrist] = 2;
///	servoChangeRate[L_Wrist] = 2;
	return;
}
...
task drive()
{
	...
	while (true)
	{
		...
		if(abs(joystick.joy1_y1) > deadband || abs(joystick.joy1_x1) > deadband)
		{
			if(abs(joystick.joy1_y1) > abs(joystick.joy1_x1))
			{
				...
				else
				{
					...
					motor[L_Front] =	-joystick.joy1_y1*cut;
					motor[R_Front] =	-joystick.joy1_y1*cut;
					motor[L_Rear] = -joystick.joy1_y1*cut;
					motor[R_Rear] = -joystick.joy1_y1*cut;
			...
			{
				motor[L_Front] =	-joystick.joy1_x1*cut;
				motor[R_Front] =	joystick.joy1_x1*cut;
				motor[L_Rear] = -joystick.joy1_x1*cut;
				motor[R_Rear] = joystick.joy1_x1*cut;
			}
		...
		if(joy1Btn( 6  ))			// If button 8 (bottom right trigger) is pressed, turn wrist one way
		{
		...
		if(joy1Btn( 8 ))			// If button 6 (top right trigger) is pressed, turn wrist the other way
		{
