...
task Arm()
{
	...
	while (true)
	{
		...
		while(abs(joystick.joy1_y2) > deadband)
		{
			...
			armSpeed = ((float)joystick.joy1_y2) / 10;
		...
		/*if(joy1Btn(5)) //if button 5 (top left trigger) pressed
		{
			if (!holdStarted)
			{
				armEncoder = nMotorEncoder[Arm_motor];
				holdStarted = true;
			}

			if (!motorStarted && abs(armEncoder - nMotorEncoder[Arm_motor])>5)
			{
				motorStarted = true;
				holdDirection = (armEncoder - nMotorEncoder[Arm_motor]);
			}
			else if (motorStarted)
			{
				motor[Arm_motor] += holdDirection;//(armEncoder - nMotorEncoder[Arm_motor]);
			}

			nxtDisplayCenteredBigTextLine(4, "%d", holdDirection);
			nxtDisplayCenteredBigTextLine(6, "%d", motor[Arm_motor]);
			//nxtDisplayCenteredBigTextLine(8, "%b", holdStarted);
		}
		else
		{
			holdStarted = false;
...
task drive()
{
	...
	while (true)
	{
		...
		if(joy1Btn( 1 ))			// If button 1 (A) is pressed, stop wrist the other way
		{
			servo[L_Wrist] = ServoValue[L_Wrist];
			servo[R_Wrist] = ServoValue[R_Wrist];
		}
