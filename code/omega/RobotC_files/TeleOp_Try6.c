...
#pragma config(Motor,  mtr_S1_C2_2,     R_Rear,        tmotorTetrix, openLoop, reversed)
...
task Arm()
{
	...
	while (true)
	{
		...
		{
			motor[Arm_motor] = joystick.joy1_y2;///3;
		}
...
task drive()
{
	...
	while (true)
	{
		...
		if(abs(joystick.joy1_y1) > deadband || abs(joystick.joy1_x1) > deadband)
		{
			...
			if(abs(joystick.joy1_y1) > abs(joystick.joy1_x1))
			{
				...
				motor[L_Front] =	drivepower;
				motor[R_Front] =	drivepower;
				motor[L_Rear] = drivepower;
				motor[R_Rear] = drivepower;
	...
	if(joy1Btn( 8 ))			// If button 8 (bottom right trigger) is pressed, turn wrist one way
	{
		...
		{
			servo[L_Wrist] = ServoValue[L_Wrist]-3;
			servo[R_Wrist] = ServoValue[R_Wrist]+3;
		}
	}
