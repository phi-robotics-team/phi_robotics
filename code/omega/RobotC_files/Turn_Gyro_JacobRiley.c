...
void Turn( float turnDegrees )
{
 ...
 // motor[F_L] = -rotDirection * motorSpeed;
  ...
  while( ( abs(turnDegrees) - abs(Angle) ) > 3 )
  {
    ...
    //motor[F_L] = rotDirection * motorSpeed ; // looking from the top, left side moves back, right
    motor[Right_Side] = -rotDirection * motorSpeed ; // side moves forward for (+) CCW rotation
    motor[Left_Side] = -rotDirection * motorSpeed ; // side moves forward for (+) CCW rotation
