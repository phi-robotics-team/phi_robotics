...
void Arm_move( float Degrees )
{
  ...
  int max_speed = 25;
  nMotorPIDSpeedCtrl[Block_Arm]= mtrSpeedReg;
...
nMotorPIDSpeedCtrl[Block_Arm] = mtrSpeedReg;
  ...
  motorSpeed = 5;
  motor[Block_Arm] = direction * motorSpeed;
  wait1Msec(10);
  ...
  nMotorEncoder[Block_Arm] = 0;

//  nMotorEncoderTarget[F_L] = countDistance;
 // nMotorEncoderTarget[F_R] = countDistance;

  motor[Block_Arm] = direction * MAX_ENCODER_SPEED;



  while (abs(nMotorEncoder[Block_Arm]) < countDistance) //while the encoder wheel turns one revolution
  {
    // This condition waits for motors D & E to come to an idle position. Both motors stop
    // and then jumps out of the loop
  }

  motor[Block_Arm] = -direction*75;
