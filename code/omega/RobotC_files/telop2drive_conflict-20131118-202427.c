 ...
 float drivepower;
  int cut = 10;
	while (true)
	{
		...
		//Watches for button presses
		if(joy1Btn( BTN_LB ))
		{
			motor[Winch] = -50; //When left bumper is pressed, move winch at -50% power
		}
		...
		if(joy1Btn( BTN_SELECT )) //If the select button is pressed
		{
			if (mode == 1) //If it is in mode 1
			{
				mode = 2; //Change to mode 2
			}
			else if (mode == 2) //If it is in mode 2
			{
				mode = 1; //Change to mode 1
			}
			else //If for some reason it's not either mode
			{
				mode = 1; //Set to the default, mode 1
			}
		}
	}
}
